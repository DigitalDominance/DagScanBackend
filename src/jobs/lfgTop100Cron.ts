...
async function snapshotTop(tokens: any) {
  ...
  const tokenOps = tokens.map((it: any) => {
    ...
    return {
      updateOne: {
        ...
        update: {
          $set: {
            deployerAddress: it.deployerAddress?.toLowerCase?.() ?? it.deployerAddress,
            ticker: it.ticker,
            name: it.name,
  ...
  // function" errors.
  if (tokenOps.length && typeof (LFGToken as any).bulkWrite === 'function') {
    await LFGToken.bulkWrite(tokenOps, { ordered: false })
  }
  if (priceOps.length && typeof (LFGTokenPrice as any).bulkWrite === 'function') {
    await LFGTokenPrice.bulkWrite(priceOps, { ordered: false })
  }
